...
*No page numbers to guess. No manual splitting. Local by default — Drive export is opt-in.*
...
| Runs with zero network calls by default | Parsing, OCR, splitting, naming, ZIP export — all local. Only optional Drive export talks to the network, and only after you connect it |
| Exports split sections as PDF *or* DOCX | Round-trips DOCX → PDF (for detection) → back to real DOCX per section |
| Sends split files straight into a Drive folder | Optional, opt-in Google sign-in (`drive.file` scope — app-created files only) |
...
- [Setting Up Google Drive Export](#%EF%B8%8F-setting-up-google-drive-export)
- [DOCX Handling — How It Actually Works](#-docx-handling--how-it-actually-works)
- [Privacy](#-privacy)
...
## ☁️ Setting Up Google Drive Export

Drive export requires a one-time developer setup — Google requires every app that
requests Drive access to have its own registered OAuth Client ID. This can't be
pre-generated for you; it's tied to your own Google Cloud account. Takes about 5 minutes:
...
3. **APIs & Services → OAuth consent screen** → choose **External** (unless you have a
   Workspace org) → fill in the required app name/support email fields → add the scope
   `https://www.googleapis.com/auth/drive.file` → save. While in testing mode, add your
   own Google account under **Test users** so you can sign in.
4. **APIs & Services → Credentials → Create Credentials → OAuth client ID** → Application
   type: **Chrome Extension** → for the **Item ID**, use the extension ID shown at
   `chrome://extensions` after you've loaded this extension unpacked once (it looks like
   `phfgmcembldkeidfkalcdjmbdapahcdj`).
5. Copy the generated Client ID (ends in `.apps.googleusercontent.com`).
6. Open `public/manifest.json` in this project and replace the placeholder:
   ```json
   "oauth2": {
   ...
   ```
7. `npm run build`, then reload the extension at `chrome://extensions`.
8. Open the popup → process a document → expand **Google Drive** in the sidebar →
   **Connect Google Drive** → pick or create a folder → export.

**Why `drive.file` and not full Drive access?** This scope only grants the extension
visibility into files/folders it creates itself — it cannot browse, read, or modify
anything else already in your Drive. It also avoids Google's more invasive app
verification process required for broader scopes.

**If "Connect" fails with an auth error**, it almost always means the Client ID is still
the placeholder value, or the extension ID used when creating the OAuth client doesn't
match the extension's actual loaded ID (unpacked extension IDs can change if you move/
re-zip the folder — recreate the OAuth client with the current ID if this happens).

---

## 📝 DOCX Handling — How It Actually Works

Earlier versions of this extension tried to read `.docx` files by passing the same raw
file buffer into two separate Mammoth.js calls back to back. In some browser/bundler
combinations, the first call reads (and in effect exhausts) that buffer, so the second
call silently got nothing — which is why dropping a `.docx` file used to just make it
disappear with no visible error. That's fixed now: every Mammoth call gets its own
independent copy of the file bytes, and any real read failure now surfaces as a clear
on-screen error instead of failing silently.

The DOCX pipeline is a full round trip, entirely local:

```
.docx dropped
     │
     ▼
Mammoth.js extracts raw text (fresh buffer copy)
     │
     ▼
Text is laid out into a real paginated PDF (pdf-lib)
— this is what makes DOCX go through the exact same
  detection/splitting engine as native PDFs
     │
     ▼
Regex/OCR detection + splitting runs exactly as normal
     │
     ▼
Export format = DOCX?
     │
     ├─ No  → split PDF sections exported directly
     │
     └─ Yes → each split section's page range is re-extracted
              as text, and rebuilt into a real, fresh .docx
              file per section (via the `docx` library) —
              headings inside each section are re-styled as
              real Word heading paragraphs, not just plain text
```

This means a `.docx` manual can now be exported back out as either split **PDFs** or
split **DOCX** files, your choice, via the export format dropdown in the toolbar.

---

## 🔒 Privacy

- **No `host_permissions`** to any website — the extension cannot read pages you visit.
- **The core pipeline makes zero network calls.** Parsing, detection, OCR, splitting,
  and PDF/DOCX export all run inside the extension's own sandboxed page, using
  WebAssembly (PDF.js, Tesseract.js) and pure JS (pdf-lib, Mammoth.js, `docx`).
- **Google Drive export is the one opt-in exception.** It's off by default, requires
  you to explicitly click "Connect Google Drive" and sign in, and only then does the
  extension talk to `www.googleapis.com` — solely to upload the files you choose to
  export, into a folder you choose. It uses the `drive.file` OAuth scope, so it can
  only see/manage files it creates itself, never browse or read the rest of your Drive.
  If you never click Connect, no Drive-related network call ever happens.
- Permissions used: `storage` (settings/recent files), `downloads` (saving local
  output), `unlimitedStorage` (large manuals), `identity` (Drive sign-in, only invoked
  on demand), `host_permissions` for `googleapis.com` (Drive API calls, only when
  connected).
- The other caveat: Tesseract.js OCR language packs are not bundled by default
  (~10–15MB each) and will fetch from Tesseract's CDN the first time OCR runs, unless
  you vendor them locally — see the note in-app under Settings.

---

## ⚠️ Known Limitations

- DOCX → PDF conversion (used internally for detection, and for PDF exports of DOCX
  sources) is plain-text layout, not a pixel-faithful re-render of the original Word
  formatting. DOCX exports rebuild headings as real Word styles, but complex tables,
  images, and custom styling from the source file are not preserved.
- "Compression" repacks PDF objects (pdf-lib object streams) rather than recompressing
  embedded images — expect modest, not aggressive, size reduction.
- Google Drive export requires a one-time Google Cloud OAuth Client ID setup per
  deployment — see [Setting Up Google Drive Export](#%EF%B8%8F-setting-up-google-drive-export).
  It won't work out of the box until that's configured.
...
- [x] ZIP or individual export, as PDF or DOCX
- [x] DOCX round-trip (DOCX → PDF detection → back to real DOCX per section)
- [x] Google Drive export — connect, pick/create folder, upload (needs your own OAuth Client ID)
- [ ] Bundled offline OCR language packs (opt-in per language)
- [ ] Code-split the OCR path to shrink initial bundle size
- [ ] Resumable Drive uploads for very large manuals
